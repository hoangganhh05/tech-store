...
Các hàng đơn và địa chỉ giao hàng là bản chụp tại thời điểm đặt đơn. Bảng
`inventories` là nguồn tồn kho vận hành duy nhất; `product_variants.stock_quantity`
chỉ là projection tương thích của số khả dụng. Tồn kho có optimistic locking và
mỗi biến động được ghi vào `inventory_transactions`.
...
role và dữ liệu nền tảng. Tài khoản `admin@techstore.local` lịch sử bị khóa ở
V26; production tạo Admin đầu tiên từ `INITIAL_ADMIN_*` private secret. Không
chạy đồng thời script schema cũ trên database đã được Flyway quản lý. Kiểm tra nhanh:
