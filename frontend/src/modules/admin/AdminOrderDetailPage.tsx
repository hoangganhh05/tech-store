  ...
  return (
    <Stack spacing={3}>
      <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', sm: 'center' }} spacing={1}>
        <PageIntro eyebrow="Quản trị" title="Chi tiết đơn hàng" description="Kiểm tra đầy đủ thông tin đơn hàng trước khi xử lý." />
        <Button component={RouterLink} to={ROUTES.adminOrders} variant="outlined">Quay lại đơn hàng</Button>
      </Stack>
      ...
      {!loading && !error && order && (
        <>
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={3}>
            <Card sx={{ flex: 1 }}>
              <CardContent>
                <Typography variant="h6" component="h2" mb={1.5}>Khách hàng</Typography>
                <Typography fontWeight={600}>{order.customer.fullName}</Typography>
                <Typography>{order.customer.phone || 'Chưa cập nhật số điện thoại'}</Typography>
                <Typography color="text.secondary">{order.customer.email}</Typography>
              </CardContent>
            </Card>
            <Card sx={{ flex: 1 }}>
              <CardContent>
                <Typography variant="h6" component="h2" mb={1.5}>Thanh toán</Typography>
                <Typography>{paymentMethodLabels[order.paymentMethod] || order.paymentMethod}</Typography>
                <Typography color="text.secondary">{paymentStatusLabels[order.paymentStatus] || order.paymentStatus}</Typography>
              </CardContent>
            </Card>
          </Stack>
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={3}>
            <Card sx={{ flex: 1 }}>
              <CardContent>
                <Typography variant="h6" component="h2" mb={1.5}>Địa chỉ giao hàng</Typography>
                {order.shippingAddress ? (
                  <>
                    <Typography fontWeight={600}>{order.shippingAddress.recipientName}</Typography>
                ...
                ) : <Typography color="text.secondary">Không có thông tin địa chỉ giao hàng.</Typography>}
              </CardContent>
            </Card>
            <Card sx={{ flex: 1 }}>
              <CardContent>
                <Typography variant="h6" component="h2" mb={1.5}>Ghi chú nội bộ</Typography>
                <Typography color={order.internalNote ? 'text.primary' : 'text.secondary'}>
          ...
          <Card>
            <CardContent>
              <Typography variant="h6" component="h2" mb={2}>Lịch sử trạng thái</Typography>
              <Divider sx={{ mb: 2 }} />
