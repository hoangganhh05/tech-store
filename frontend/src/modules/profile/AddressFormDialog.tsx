...
export function AddressFormDialog({ open, editing, onClose, onSaved }: Props) {
  ...
  const handleChange =
    (field: AddressField) => (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const validate = (): FieldErrors => {
    ...
    else if (values.recipientName.trim().length > 150)
      errors.recipientName = "Tên không được vượt quá 150 ký tự.";
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = isAxiosError<{ message?: string }>(error)
        ...
        : undefined;
      setSubmitError(
        message || "Không thể lưu địa chỉ lúc này. Vui lòng thử lại sau.",
      );
    } finally {
