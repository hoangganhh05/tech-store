...
const emptyValues: UpdateProfilePayload = {
  fullName: "",
  phone: "",
  dateOfBirth: null,
};
...
export function ProfilePage() {
  ...
  const handleChange =
    (field: ProfileField) => (event: ChangeEvent<HTMLInputElement>) => {
      setValues((current) => ({
        ...current,
        [field]: event.target.value || (field === "dateOfBirth" ? null : ""),
      }));
  ...
  const parseServerErrors = (message: string): FieldErrors => {
    ...
    message.split(";").forEach((part) => {
      ...
      if (field in emptyValues)
        errors[field] = part.slice(separator + 1).trim();
    });
    return errors;
  };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (Object.keys(serverErrors).length > 0) setFieldErrors(serverErrors);
        ...
        setSubmitError(
          message ||
            "Không thể cập nhật thông tin lúc này. Vui lòng thử lại sau.",
        );
    } finally {
