...
const mockedUpdateCategoryDisplay = vi.mocked(updateCategoryDisplay);
...
describe("AdminCategoriesPage", () => {
  ...
  it("toggles category visibility via switch", async () => {
    mockedUpdateCategoryDisplay.mockResolvedValue({
    ...
    const phoneSwitch = toggleSwitches[0];
    expect(phoneSwitch).toBeChecked();

    fireEvent.click(phoneSwitch);

    await waitFor(() => {
      expect(mockedUpdateCategoryDisplay).toHaveBeenCalledWith(1, {
        displayOrder: 0,
        isActive: false,
      });
    });
  ...
  it("updates display order inline on blur", async () => {
    mockedUpdateCategoryDisplay.mockResolvedValue({
      ...mockFlatCategories[0],
      displayOrder: 5,
    });
    ...
    const phoneOrderInput = inputs[0];
    expect(phoneOrderInput).toHaveValue(0);

    fireEvent.change(phoneOrderInput, { target: { value: "5" } });
    fireEvent.blur(phoneOrderInput);
